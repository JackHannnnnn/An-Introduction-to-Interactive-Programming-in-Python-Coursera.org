...
def group_collide(group,other_object):
    ...
    for i in set(group):
        if i.collide(other_object):
            ...
            return True
    return False
def group_group_collide(missile__group,rock__group):
    global num_of_hit, score
    num_of_hit = 0
    for i in set(missile__group):
        if group_collide(rock__group,i):
           missile__group.remove(i)
           num_of_hit += 1
    score += num_of_hit
...
class Sprite:
    ...
    def draw(self, canvas):
        if not self.animated:
            ...
            canvas.draw_image(explosion_image,[64 + 128*(self.age//2),64],[128,128],self.pos,self.image_size,self.angle)
...
def rock_spawner():
    ...
    if started:
        if len(rock_group) < 12:
            if dist(my_ship.get_position(), rock_pos) > 100:
